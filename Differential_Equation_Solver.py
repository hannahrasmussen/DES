...
def evaluate_delta(x,y,y5,y4,der,dx):
    delta_array = abs(y5-y4)
    delta = max(delta_array)
    delta_ideal = eps*(abs(y)+abs(.5*((y5-y)+(y4-y)))+10**-10) #OK so the problem is that delta is not 0 but delta_ideal is
    if delta==0:
        return "good", delta
    else:
        index = np.nonzero(delta_array)
    if min(delta_ideal[index]/delta_array[index])>1: #checks to make sure EVERY delta is less than the maximum delta
        return "good", min(delta_ideal[index]/delta_array[index]) 
    else:
        if(min(delta_ideal[index])==0):
            def Cash_Karp (x,y,der,dx):
                print("dx is "+str(dx))
                k1 = dx*der(x,y)
                print("k1 is "+str(k1[22]))
                k2 = dx*der(x+(a2*dx),y+(b21*k1))    
                print("k2 is "+str(k2[22]))
                k3 = dx*der(x+(a3*dx),y+(b31*k1)+(b32*k2))
                print("k3 is "+str(k3[22]))
                k4 = dx*der(x+(a4*dx),y+(b41*k1)+(b42*k2)+(b43*k3))
                print("k4 is "+str(k4[22]))
                k5 = dx*der(x+(a5*dx),y+(b51*k1)+(b52*k2)+(b53*k3)+(b54*k4))
                print("k5 is "+str(k5[22]))
                k6 = dx*der(x+(a6*dx),y+(b61*k1)+(b62*k2)+(b63*k3)+(b64*k4)+(b65*k5))
                print("k6 is "+str(k6[22]))
                y5th = y+(c1*k1)+(c2*k2)+(c3*k3)+(c4*k4)+(c5*k5)+(c6*k6) 
                y4th = y+(c10*k1)+(c20*k2)+(c30*k3)+(c40*k4)+(c50*k5)+(c60*k6)
            print(delta_ideal[index])
            print(y[22])
            print(y5[22])
            print(y4[22])
            Cash_Karp(x,y,der,dx)
        return "bad", min(delta_ideal[index]/delta_array[index])
...
def driver(x,y,der,dx):
    for i in range(10): #don't want it to go through this loop more than 10 times
        ...
        gb, d = evaluate_delta(x,y,y5,y4,der,dx) #d is delta if delta=0, but d is delta_ratio otherwise
        if gb=="good":
            if d==0:
                dx_new = dx*5
            else: 
